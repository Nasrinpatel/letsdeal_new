...
class Leadmaster_model extends CI_model
{
    ...
    function all_property_suggestion($search_params=[]){
        $alldata = array();
        if (!empty($search_params)) {
            if (isset($search_params['start_date']) && !empty($search_params['start_date'])) {
        ...
        $a = $this->db->where('tb_property_master.thumbs_up', 0)
    ...
    function getFollowups($id){
        $data = $this->db->get_where('tb_followup_master',['model_type'=>'Lead','model_id'=>$id])->result_array();
        return $data;
    }
