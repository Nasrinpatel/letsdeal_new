...
class Customermaster extends CI_Controller
{
	...
	{
		$followups = $this->customermaster->getFollowups($id);
		$result = array('data' => []);
		$i = 1;
		foreach ($followups as $value) {

			$type_data = $this->db->get_where('tb_followup_type_master', array('id' => $value['followtype_id']))->row();
			$button = '<a href="' . base_url('admin/customermaster/edit_followups/' . $value['id']) . '" class="action-icon edit-btn" data-id="' . $value['id'] . '" data-bs-toggle="modal" data-bs-target="#edit-customer-followup-modal"><i class="mdi mdi-square-edit-outline text-warning"></i></a>
			<a href="' . base_url('admin/customermaster/delete_followups/' . $value['id'] . '/' . $id) . '#customer-followups" class="action-icon delete-btn"> <i class="mdi mdi-delete text-danger"></i></a>';
			$result['data'][] = array(
				$i++,
				// $value['type'],
				// $type_data->name,
	...
	public function store_followups()
	{
		...
		$response = $this->customermaster->save_followup_records($data);
		if ($response == true) {
			echo json_encode(array('success' => true, 'message' => 'Customer Followup Added Successfully.'));
		} else {
			echo json_encode(array('success' => false, 'message' => 'Something went wrong. Please try again'));
		}
	}
	...
	{
		$data = $this->customermaster->getFollowup($id);
		echo json_encode($data);
	}
	...
	{
		$response = $this->customermaster->delete_followups_records($id);

		if ($response == true) {
			$this->session->set_flashdata('success', 'Customer Followup Deleted Successfully.');
		} else {
			$this->sesssion->set_flashdata('error', 'Something went wrong. Please try again');
		}
		return redirect('admin/Customermaster/addfollowup/' . $customer_id);
	}
